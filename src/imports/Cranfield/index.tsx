...
const BRANCH_ADDRESS = "A converted Wesleyan chapel, Cranfield";
const GALLERY_IMAGES = Array.from({ length: 12 }, () => imgGallery);
const FACTS = [
  { label: "Bible study launched", value: "February 2022" },
  { label: "Dedicated", value: "15 October 2023" },
  { label: "Leader", value: "Rev Fungai Mangere" },
  { label: "Average attendance", value: "45" },
];
        ...
        <div className="flex min-w-0 flex-col items-center justify-center md:flex-row md:justify-start">
          ...
          <div className="min-w-0 text-center md:text-left">
            ...
            <h1 className="mt-2 text-[56px] leading-[0.86] tracking-[-0.035em] text-[#38362d] sm:text-[64px] md:text-[clamp(42px,6vw,72px)]">
              <span className="md:hidden">CRANFIELD</span>
              <span className="hidden md:inline">CRANFIELD</span>
            </h1>
          </div>
        </div>
...
function HeroImage() {
  return (
    <figure className="m-0 aspect-[1000/1500] w-full overflow-hidden bg-[#e9e7e0] md:aspect-auto md:h-[clamp(360px,48vw,580px)] md:max-h-none">
      <img
        src={imgMobileHero}
        alt="Cranfield church building exterior"
      ...
      <div className="relative mx-auto grid max-w-[1240px] gap-10 md:grid-cols-[190px_minmax(0,1fr)] md:gap-10 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-12">
        ...
        <article className="order-1 md:order-2">
          ...
          <div className="mt-8 space-y-7 text-[18px] leading-[32px] text-[#38362d]">
            ...
            <p>
              In July 2024, Superintendent General Reverend Olusola Adesope made a surprise visit with his family — a mark of how quickly this newest congregation had found its place in the wider Apostolic Faith family. Membership has since more than doubled, from twenty-two at dedication to an average of forty-five each Sunday, Sunday School teachers have grown from three to nine, and eight babies have been born into the church family — alongside healing miracles and ongoing testimonies of salvation and sanctification.
            </p>
          </div>
        </article>
      </div>
          ...
          {GALLERY_IMAGES.map((src, index) => (
            ...
            >
              <div className="aspect-[0.9/1] w-full overflow-hidden sm:aspect-[1.28/1]">
                <img
                  src={src}
                  alt={`Cranfield branch gathering ${index + 1}`}
                  className="block"
